...
@app.route('/')
def index():
    mahasiswa_hobi_data = get_mahasiswa_hobi(db)
...
@app.route('/mahasiswa')
def data_mahasiswa():
    mahasiswa_data = get_mahasiswa_data(db)
    return render_template('mahasiswa.html', mahasiswa_data=mahasiswa_data)
...
@app.route('/hobi')
def data_hobi():
    hobi_data = get_hobi_data(db)
    return render_template('hobi.html', hobi_data=hobi_data)
...
@app.route('/mhshobi')
def data_mhshobi():
    mhshobi_data = get_mhshobi_data(db)
...
def tambah_mhshobi():
    ...
    mahasiswa_data = get_mahasiswa_without_hobi(db)
    hobi_data = get_hobi_data(db)
